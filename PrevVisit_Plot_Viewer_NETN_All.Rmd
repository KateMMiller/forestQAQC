...
visit_notes <- joinVisitNotes(park = park, from = year, to = year, noteType = "all") |> 
  ...
  select(Plot_Name, Notes) |> group_by(Plot_Name) |> unique() |> summarize(Notes = toString(unique(Notes)))
...
comb <- purrr::reduce(list(plot_info, visit_notes, trees, regen, quadspp, addspp), left_join, by = "Plot_Name") |> 
  mutate(Concerns = NA) |> 
