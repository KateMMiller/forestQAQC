...
#params <- data.frame(yearpv = 2019, year_curr = 2024, parkcode = "WEFA", panel = 2, print = FALSE)
...
plot_info <- joinLocEvent(park = park, from = year, to = year, output = 'verbose', panel = panel) |> 
...
regen <- joinRegenData(park = park, from = year, to = year, panel = panel) |> 
  group_by(Plot_Name) |> summarize(num_saps = round(sum(sap_den) * 3, 1),
                                   num_seeds = round(sum(seed_den) * 3, 1))

shrubs <- joinMicroShrubData(park = park, from = year, to = year, panel = panel) |> 
...
quadspp <- joinQuadSpecies(park = park, from = year, to = year, panel = panel) |> 
  filter(IsGerminant == FALSE) |> 
  group_by(Plot_Name) |> 
  summarize(num_quadspp = sum(!ScientificName %in% "None present"))

addspp <- joinAdditionalSpecies(park = park, from = year, to = year, panel = panel) |> 
