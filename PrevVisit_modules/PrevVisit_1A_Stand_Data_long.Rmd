...
az_df <- data.frame(UC = c(UC, ifelse(grepl("ACAD", plot_name), 7.5, 10)), 
                    UR = c(ifelse(UC + 45 > 360, UC + 45 - 360, UC + 45),
                           ifelse(grepl("ACAD", plot_name), 10.61, 14.14)),
                    MR = c(ifelse(UC + 90 > 360, UC + 90 - 360, UC + 90),
                           ifelse(grepl("ACAD", plot_name), 7.5, 10)),
...
dist1 <- ifelse(grepl("ACAD", plot_name), 7.5, 10)
dist2 <- ifelse(grepl("ACAD", plot_name), 10.61, 14.14)
...
suppressWarnings(
  plotTreeMap(park = park, from = as.numeric(year), 
            to = as.numeric(year), plotName = plot_name)
)
```
</div>
<div class = "one">
