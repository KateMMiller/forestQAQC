...
#                "/")
# MIDN drive is mapped to X on some folks' laptops and Y on others, so making more flexible.
photo_path <- paste0("X:/Files/Monitoring/ForestVegetation/", year, "/03_Data/Photos/PhotoPoints/", park, "/")
if(!dir.exists(photo_path)){photo_path <- paste0("Y:/Files/Monitoring/ForestVegetation/", year, "/03_Data/Photos/PhotoPoints/", park, "/")}
photo_eval <- if(dir.exists(photo_path)){TRUE} else {FALSE} # better way to prevent MIDN_All.RMD from crashing
# if not connected to the MIDN server.
