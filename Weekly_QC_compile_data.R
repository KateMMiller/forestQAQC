...
dbi_diff <- full_join(stand_new %>% filter(IsQAQC == 0) %>% 
                        select(Plot_Name, Deer_Browse_Index),
                      stand_old %>% filter(IsQAQC == 0 & cycle == params$cycle_latest - 1) %>% 
                        select(Plot_Name, Deer_Browse_Index),
                      by = "Plot_Name", 
                      suffix = c("_new", "_old")) %>% 
            mutate(DBI_diff = abs(Deer_Browse_Index_new - Deer_Browse_Index_old)) %>% 
