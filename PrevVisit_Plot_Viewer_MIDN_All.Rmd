...
params:
  yearpv: 2018 # MUST BE LINKED TO DRIVE WITH PHOTOPOINTS TO WORK
  parkcode: "APCO"
  print: FALSE
---
```{r include = FALSE}
...
stand_info <- joinStandData(park = park, from = year, to = year,
                                eventType = 'all') |> 
...
trees <- joinTreeData(park = park, from = year, to = year, status = 'active') |> 
...
res <- lapply(seq_along(child_env$plot_list), function(x) {
  ...
  knitr::knit_child(
    'PrevVisit_Plot_Viewer_MIDN_Indiv.Rmd', 
     envir = child_env,
     quiet = TRUE )
