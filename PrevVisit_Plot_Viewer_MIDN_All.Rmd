...
params:
  yearpv: 2022 # MUST BE LINKED TO DRIVE WITH PHOTOPOINTS TO WORK
  parkcode: "FRSP"
  panel: 4
  print: FALSE
---
...
#           PrevVisit_5A_Photopoints_MIDN.Rmd. If the path given can't be accessed upon knit, photopoints won't be included
#           in the output.
```
...
library(htmlwidgets)
#comment out next 2 lines - for troubleshooting only
# params <- data.frame(yearpv = 2022, parkcode = "FRSP", panel = 4, print = F)
...
child_env$plot_list <- child_env$plots$Plot_Name
...
PI_count <- PI |> 
            ...
            filter(PI_quads > 0) |>  # Not really needed b/c there has to be 1 quad present for toxrad
...
concerns <- rbind(stand_conc, shrubs, cwd_conc) |> 
  group_by(Plot_Name) |> summarize(Concerns = toString(unique(Concerns)), .groups = "drop")
