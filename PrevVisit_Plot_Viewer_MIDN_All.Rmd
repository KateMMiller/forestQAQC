...
params:
  yearpv: 2021 # MUST BE LINKED TO DRIVE WITH PHOTOPOINTS TO WORK
  parkcode: "THST"
  panel: 3
  print: FALSE
---
...
#--- PrevVisit_Plot_Viewer_MIDN_All.Rmd generates the Summary tab and iterates through the park's list of plots.
#------ PrevVisit_Plot_Viewer_MIDN_Indiv.Rmd renders each plot's report within the plot-named tabs. 
#       This report shows the order of the modules that are rendered and allows you to turn them off, if they're failing.
#---------- PrevVisit_1A_Stand_Data_MIDN.Rmd (and others with numbers) are individual data modules. If you try to
#           knit the plot viewer and it fails, the error should tell you which module failed. Where NETN and MIDN can use 
#           the same module, only 1 file exists. Where there are separate modules, MIDN's module is named. 
#---------- NOTE: The photopoints, which are rendered last, must either be updated to where the photos for a given year
#           are located on the MIDN server, your laptop, or an external drive. The file that needs to be checked is
#           PrevVisit_5A_Photopoints_MIDN.Rmd
```
...
sys.source("./PrevVisit_modules/PrevVisit_FieldForms_MIDN_compile.R", envir = child_env)
```


```{r echo = F, include = F}
# Summary Table for first tab
plot_info <- joinLocEvent(park = park, from = year, to = year, panel = panel,
                                eventType = 'all') |> 
             mutate(Unit = substr(ParkSubUnit, 6, nchar(ParkSubUnit))) |> 
             select(Plot_Name, PlotCode, Unit, #Notes = PlotNotes, 
                    Directions)#, Stunted = IsStuntedWoodland)  

stand_info <- joinStandData(park = park, from = year, to = year, panel = panel,
                                eventType = 'all') |> 
  ...
  select(Plot_Name, slope, crown, rocky, midstory, understory, water, 
         water_cov) |> 
...
visit_notes <- joinVisitNotes(park = park, from = year, to = year, noteType = "all", panel = panel,
                                eventType = 'all') |> 
  ...
  mutate(Notes = ifelse(Note_Type == "Stand_Disturbances", paste0("Disturbance: ", Notes), Notes),
         order = case_when(Note_Type == "Plot_Notes" ~ 1,
                           Note_Type == "Observer_Tab_Notes" ~ 2,
                           Note_Type == "Stand_Notes" ~ 3,
                           TRUE ~ 4)) |> arrange(Plot_Name, order) |> 
  select(Plot_Name, Notes) |> group_by(Plot_Name) |> unique() |> 
  summarize(Notes = toString(unique(Notes)))

trees <- joinTreeData(park = park, from = year, to = year, status = 'active', panel = panel) |> 
  filter(!TreeStatusCode %in% "DF") |> 
  group_by(Plot_Name) |> summarize(num_trees = sum(num_stems))


regen <- joinRegenData(park = park, from = year, to = year, units = "sq.m",
                                eventType = 'all', panel = panel) |> 
  group_by(Plot_Name) |> summarize(num_saps = round(sum(sap_den) * 84.823, 1),
                                   num_seeds = round(sum(seed_den) * 12, 1))

shrubs <- joinMicroShrubData(park = park, from = year, to = year,
                                eventType = 'all', panel = panel) |> 
...
PI <- joinQuadSpecies(park = park, from = year, to = year,
                                eventType = 'all', panel = panel) |> 
                      select(Plot_Name, ScientificName, starts_with("Pct_Cov"))

PI_long <- PI |> pivot_longer(starts_with("Pct_Cov"), names_to = "Quad", values_to = "Pct_Cover")

PI_count <- PI_long %>%  group_by(Plot_Name) %>%  
                       filter(ScientificName == "Toxicodendron radicans") %>% 
                       filter(Pct_Cover > 0) %>%  
                       summarize(PI_quads = n()) %>%  
                       mutate(Concerns = paste0("PI in ", PI_quads, " quads")) |> 
                       select(Plot_Name, Concerns) |> filter(!is.na(Concerns))

PI_max <- PI_long %>%  group_by(Plot_Name) %>%  
                      filter(ScientificName == "Toxicodendron radicans") %>% 
                      filter(Pct_Cover > 0) %>%  
                      summarize(PI_max = max(Pct_Cover)) %>%  
                      mutate(Concerns = paste0("Max PI =  ", PI_max, "%")) |> 
                      select(Plot_Name, Concerns) |> filter(!is.na(Concerns))  

PI_mean <- PI_long %>%  group_by(Plot_Name) %>%  
                      filter(ScientificName == "Toxicodendron radicans") %>% 
                      filter(Pct_Cover > 0) %>%  
                      summarize(PI_avg = round((sum(Pct_Cover)/12), 1)) %>%  
...
cwd_conc <- VIEWS_MIDN_NCBN$CWD_MIDN_NCBN |> filter(ParkUnit == park) |> filter(SampleYear == year) |> 
  filter(PanelCode == panel) |> 
  group_by(Plot_Name, SampleYear) |> summarize(num_cwd = sum(!is.na(Distance))) |>
...
concerns <- rbind(stand_conc, shrubs, cwd_conc, PI_count, PI_max, PI_mean) |> 
  group_by(Plot_Name) |> summarize(Concerns = toString(unique(Concerns)))

comb <- purrr::reduce(list(plot_info, visit_notes, trees, regen, quadspp, addspp, concerns), 
                      left_join, by = "Plot_Name") |> 
  select(PlotCode, Unit, 
...
comb_tbl <- datatable(comb, class = 'cell-border stripe', rownames = FALSE,
                      ...
                      colnames = c("Plot", "Unit", "Trees", "Saps", "Seeds",
                                   "QuadSpp", "AddSpp", 
...
res <- lapply(seq_along(child_env$plot_list), function(x) {
  ...
  knitr::knit_child(
    './PrevVisit_modules/PrevVisit_Plot_Viewer_MIDN_Indiv.Rmd', 
     envir = child_env,
     quiet = TRUE )
