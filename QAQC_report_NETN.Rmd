...
shrub_tab_ta <- if(nrow(shrub_spp) > 0){ # this is prelim df to shrub_spp_wide
                   kable(shrub_spp_wide %>% select(ScientificName, crew, qaqc),
                   ...
                   column_spec(1:3, border_right = TRUE, border_left = T, 
                               background = case_when(shrub_spp_wide$missed_c == 1 ~ "#ACC1DE",
                                                      shrub_spp_wide$missed_q == 1 ~ "#DEACAC",
                                                      TRUE ~ "#ffffff")) %>% 
                   row_spec(c(0, nrow(shrub_spp_wide)-1), extra_css = "border-bottom: 1px solid #000000;") %>% 
                   row_spec(nrow(shrub_spp_wide), bold = T, extra_css = "border-bottom: 1px solid #000000;",
                             background = ifelse(shrub_spp_wide[nrow(shrub_spp_wide), 3] < 0.9, diff_bad, "#ffffff"))
                   } 
