...
forestNETN::importCSV(path = "D:/NETN/R_Dev/data/", zip_name = "NETN_Forest_20220316.zip") # zip after new views validated
...
# Error check:
qaqc <- forestNETN::joinLocEvent(QAQC = TRUE) %>%
...
sim_cols <- c("#ffffff", "#ffffff", ifelse(!is.na(seeds_spp_wide2[nrow(seeds_spp_wide2), 3]) &
                                             seeds_spp_wide2[nrow(seeds_spp_wide2), 3] < 0.9, diff_bad, "#ffffff"),
              "#ffffff", ifelse(!is.na(seeds_spp_wide2[nrow(seeds_spp_wide2), 5]) &
                                  seeds_spp_wide2[nrow(seeds_spp_wide2), 5] < 0.9, diff_bad, "#ffffff"))

seeds_tab_ta <- if(nrow(seeds_spp) > 0){ # this is prelim df to seeds_spp_wide
                   kable(seeds_spp_wide2 %>% select(ScientificName, sds_15_crew, sds_15_qaqc,
                                                    sds_30_crew, sds_30_qaqc),
                         format = 'html', align = 'c', 
                         col.names = c("Species", "Crew 15-30cm", "QAQC 15-30cm", 
                                       "Crew >= 30cm", "QAQC >= 30cm")) %>% 
                   kable_styling(fixed_thead = TRUE, bootstrap_options = 'condensed',
                                 full_width = FALSE, position = 'left', font_size = 14) %>% 
                   ...
                   row_spec(nrow(seeds_spp_wide2), extra_css = "border-bottom: 1px solid #000000;") %>% 
                   column_spec(2:3, background = ifelse(!is.na(seeds_spp_wide2[nrow(seeds_spp_wide2), 3]) &
                                                      seeds_spp_wide2[nrow(seeds_spp_wide2), 3] < 0.9, diff_bad, "#ffffff"),
                               bold = T, extra_css = "border-bottom: 1px solid #000000;") %>%
                   column_spec(4:5, background = ifelse(!is.na(seeds_spp_wide2[nrow(seeds_spp_wide2), 5]) &
                                                      seeds_spp_wide2[nrow(seeds_spp_wide2), 5] < 0.9, diff_bad, "#ffffff"),
                               bold = T, extra_css = "border-bottom: 1px solid #000000;")
...
seeds_tab_ta
seeds_tab_ta_print <- ifelse(nrow(seeds_spp) > 0, TRUE, FALSE)
seeds_txt_ta_print <- ifelse(nrow(seeds_spp) == 0, TRUE, FALSE)
...
cat("No shrub species >0.1% cover in either visit.", "\n")
```

</details>

<details open><summary><h3>Seedlings</h3></summary>
...
cat("No seedling species in either visit.", "\n")
```
