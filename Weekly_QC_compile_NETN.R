...
quad_tramp2 <- quad_tramp %>% mutate(Plot_Name  = paste(ParkUnit, stringr::str_pad(PlotCode, 3, side = 'left', "0"), sep = "-")) %>% 
    filter(IsQAQC == 0) %>% filter(Plot_Name %in% new_evs_list) %>% select(Plot_Name, StartYear, QuadratCode, SQQuadCharCode, IsTrampled) %>% unique()

quad_join <- joinQuadData() %>% select(Plot_Name, StartYear, cycle) %>% unique()

quad_tramp3 <- left_join(quad_tramp2, quad_join, by = c("Plot_Name", "StartYear")) %>% filter(StartYear >= (curr_year - 5))

quad_tramp_wide <- quad_tramp3 %>% select(Plot_Name, StartYear, cycle, QuadratCode, IsTrampled) %>% pivot_wider(names_from = c("QuadratCode"), 
                                                                                                     values_from = "IsTrampled")
                                                                                              
quad_tramp_wide2 <- full_join(quad_tramp_wide %>% filter(cycle %in% (params$cycle_latest_num - 1)) %>% select(-cycle, -StartYear),
                              quad_tramp_wide %>% filter(cycle %in% params$cycle_latest_num) %>% select(-cycle, -StartYear),
                              by = c("Plot_Name"),
                              suffix = c("_C3", "_C4"))
...
quad_tramp_diff <- quad_tramp_wide3 %>% filter(UC_dif > 0 | UR_dif > 0 | MR_dif > 0 | BR_dif > 0 | BC_dif > 0 | BL_dif > 0 | ML_dif > 0 | UL_dif > 0)

QC_table <- rbind(QC_table, QC_check(quad_tramp_diff, "Quadrat", "Trampled in cycle 4 != cycle 3"))

tramp_plots <- quad_tramp_diff$Plot_Name

tramp_plots2 <- quad_tramp_wide %>% filter(Plot_Name %in% tramp_plots)
